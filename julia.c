...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/07 15:20:38 by kganeriw          #+#    #+#             */
/*   Updated: 2024/02/09 18:05:54 by kganeriw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	julia_n(t_data *data)
{
	...
	i = 0;
	while (i < data->max_n && data->newre * data->newre + data->mlx_new_image
		* data->newim)
	{
		...
		data->newre = data->oldre * data->oldre - data->oldim * data->oldim
			+ data->cre;
		data->newre = 2 * data->oldre * data->oldim + data->cim;
		i++; 
	}
	return (i);
}
...
void	julia(t_data *data)
{
	...
	while (y < data->width)
	{
		while (x < data ->width)
		{
			...
			data->newim = (y - data->height / 2 + data->certre_y * data->height
